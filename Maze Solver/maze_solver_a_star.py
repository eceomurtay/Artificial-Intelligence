...
def generate(num, heuristic):
    ...
    a_star = aStar.AStar(start, goal)
    start_time = timer()
    a_star_path = a_star.search(maze_graph, heuristic)
    end_time = timer()

    draw_solution(a_star_path)
    run_program()

    print("Maze ", num, "\n")
    print("Time taken to find the path: ", end_time - start_time, "secs")
    print("Length of the path: ", len(a_star_path))
    print("Number of expanded nodes: ", len(a_star.expanded))
    print("-------------------")

    delete(a_star_path)


generate(1, "euclid")               # first maze uses euclidean distance
generate(2, "manhattan")            # second maze uses manhattan distance
