...
def generate(num):
    ...
    iterative_deep = ids.IDS(start, goal, maze_graph)
    start_time = timer()
    ids_path = iterative_deep.iddfs(1000)
    end_time = timer()

    if ids_path is None:

        print("Maze ", num, "\n")
        print("Maximum depth is reached!")
        print("Number of expanded nodes: ", len(iterative_deep.expanded_nodes))
        print("-------------------")
    else:

        draw_solution(ids_path)
        run_program()

        print("Maze ", num, "\n")
        print("Time taken to find the path: ", end_time - start_time, "secs")
        print("Length of the path: ", len(ids_path))
        print("Number of expanded nodes: ", len(iterative_deep.expanded_nodes))
        print("-------------------")
...
"""
Depth can be decremented by 5 instead of 1 in each iteration to not give RecursionError
"""
