...
def subsets(iterable):
    li = list(iterable)
    return frozenset(frozenset(x) for x in (itertools.chain.from_iterable(itertools.combinations(li,n)
...
class Problem:
    ...
    def _init_constraint_dict(self):
        vars_subsets = subsets(self.variables)
        ...
        if not current_solution or self.check_constraints(current_constraints, current_solution):
            ...
            if set(current_variables) == set(self.variables):
                ...
                for value in v.d:
                    ...
                    solution[v] = value
                    valid_solutions_iter, returns_number_iter, nodes_number_iter = \
                        self._backtrack_all_solutions(solution, constraint_levels)
